...
const Card = ({ id, title, description, image }: CardProps) => {
  ...
  return (
    <article
      className={`select-none group shadow shadow-gray-600 flex h-full flex-col rounded-2xl border border-white/40 bg-white/5 p-6 backdrop-blur-xl transition-transform duration-300 hover:-translate-y-1 ${
    ...
    >
      ...
      <div className="relative mb-6 aspect-square overflow-hidden rounded-xl border border-white/5 ">
        <Image
          ...
          className="object-cover opacity-80 transition-all duration-500 group-hover:scale-105 group-hover:opacity-100 active:opacity-100 pointer-events-none"
        />
      </div>
